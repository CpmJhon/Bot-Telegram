...
if (BOT_TOKEN == '7902101355:AAHROCx5bKgR_H4x2yTSIZkNXw9qVY4iGOg') {
...
async function startalfixd() {
    bot.on('callback_query', async (alfixd) => {
        ...
        const username = user.username ? user.username : "MIKUMIKI";
    ...
    await alfixd.reply(lang.first_chat(BOT_NAME, user.full_name), {
        ...
        reply_markup: {
            inline_keyboard: [
                [{
                    text: 'My Youtube',
                    url: "https://www.youtube.com/@galeri_car_cpm8048"
                }, {
                    text: 'OWNER 😁',
                    url: OWNER[0]
                }]
...
    const isAdmin = adminUsers.includes(String(alfixd.from.id));
    if (!isAdmin) {
        await alfixd.reply('Perintah Hanya Untuk Owner, Hubungi Admin Saya Untuk Menjadi Owner atau Users Premium...', Markup.inlineKeyboard([
            [Markup.button.url('HUBUNGI ADMIN', 'https://t.me/MIKUMIKI')]
        ]));
        return;
    }
    ...
    try {
        ...
        for (let user of users) {
            ...
            if (u.root_admin) {
                ...
                messageText += 'Script By Cpm_Jhon\n';
            }
        }
...
    const isAdmin = adminUsers.includes(String(alfixd.from.id));
  ...
  if (!isAdmin) {
    alfixd.reply('Perintah Hanya Untuk Owner, Hubungi Admin Saya Untuk Menjadi Owner atau Users Premium...', {
      reply_markup: {
        inline_keyboard: [
          [{ text: 'HUBUNGI ADMIN', url: 'https://t.me/MIKUMIKI' }]
        ]
      }
    });
    return;
...
┃➥  Password : ${password} 
┗━━━━━[ Cpm_Jhon STORE  ]━━━━
THANKS FOR YOUR SUDAH BELI PANEL DI Cpm_Jhon
ADA KENDALA CHAT Cpm_Jhon YA `,
      });
